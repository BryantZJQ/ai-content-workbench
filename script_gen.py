...
PLATFORMS = {
    ...
    "抖音": {
        ...
        "constraints": (
            "【抖音算法适配】\n"
            "- 完播率是核心指标：前3秒必须有强钩子（悬念/冲突/反常识），不留任何废话\n"
            "- 每句话不超过20字，信息密度高，节奏快，每5-8秒要有一个新信息点或情绪转折\n"
            "- 中间不能有'尿点'：如果有解释性段落，必须穿插金句或反转保持张力\n"
            "- 结尾必须有强互动引导（提问/争议/投票），评论量直接影响推荐\n"
            "- 总时长15-60秒最佳，超过60秒完播率骤降\n"
            "- 避免平铺直叙，多用'你知道吗''但是''没想到'等转折词制造节奏感"
        ),
    },
    "B站": {
        "aspect_ratio": "16:9",
        "constraints": (
            "【B站算法适配】\n"
            "- 播放时长和互动率（投币/收藏）是核心指标\n"
            "- 开场可以有5-10秒人设铺垫，但必须在15秒内切入正题\n"
            "- 允许长句和复杂逻辑推理，B站用户接受知识密度更高的内容\n"
            "- 正文要有个人观点和深度分析，不能只是信息搬运\n"
            "- 适当加入'弹幕梗'和互动点（如'到这里你可能会问...'），提升弹幕量\n"
            "- 2-5分钟是黄金时长，结尾引导'一键三连'要自然不刻意\n"
            "- 标题可以更长（20-30字），允许用【】标注关键信息"
        ),
    },
    "小红书": {
        "aspect_ratio": "3:4",
        "constraints": (
            "【小红书算法适配】\n"
            "- 收藏率和分享率是核心指标，内容要有'工具价值'让人想存下来\n"
            "- 开场必须有强烈的个人体验感，用'我'开头，像闺蜜分享\n"
            "- 正文用清单/步骤/对比结构，方便快速浏览和截图收藏\n"
            "- 多用情绪词和信任词：'亲测''绝了''真的会谢''姐妹们'\n"
            "- 关键词自然嵌入（不要堆砌），方便搜索发现\n"
            "- 30-60秒最佳，超过90秒完播率很低\n"
            "- 结尾用'你们还有什么好物/方法推荐？'引导评论"
        ),
    },
}
